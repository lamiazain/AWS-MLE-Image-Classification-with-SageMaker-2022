...
def train(args,model, train_loader,test_loader, criterion, optimizer,hook):
...
def create_train_data_loaders(batch_size,DataDir):
    ...
    dataset = datasets.CIFAR10(DataDir, train=True,transform = transform_train)

    return torch.utils.data.DataLoader(dataset,batch_size=batch_size,shuffle=True)


def create_test_data_loaders(test_batch_size,DataDir):
    
    #This is an optional function that you may or may not need to implement
    #depending on whether you need to use data loaders or not
    
    dataset=datasets.CIFAR10(DataDir,train=False, transform=transform_valid)
...
def main(args):
    ...
    #
    model = train(args, model, train_loader, test_loader, loss_criterion, optimizer,hook)
